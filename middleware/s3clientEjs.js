...
exports.getS3Middleware = (fieldNames) => {
    return (req, res, next) => { // Add 'next' as an argument here
      ...
      Promise.all(fieldPromises)
        .then(() => {
          next(); // Invoke 'next' here to move to the next middleware or route handler
        })
        .catch((error) => {
          console.log("upload", error);
          next(error); // Pass the error to 'next' if an error occurs
        });
    };
  };
